...
import { stationCreateSchema } from "@/lib/schemas/station";
...
export const GET: APIRoute = async (context) => {
  ...
  let queryBuilder = supabase.from("stations").select("id, name");
  if (query) {
...
export const POST: APIRoute = async (context) => {
  ...
  if (error) {
    if (error.code === "23505") {
      return Response.json({ error: "A station with this name already exists" }, { status: 409 });
    }
    return Response.json({ error: error.message }, { status: 500 });
  }
